		...
		if ( ! empty( $rcs_settings['enable_currency_switcher'] ) && empty( $rcs_settings['app_id'] ) ) {
			...
			<div class="notice notice-warning is-dismissible">
				<p>
					<?php
					printf(
						esc_html__( '%1$sCurrency Switcher App ID%2$s is missing in %1$sRealHomes > Currency Settings%2$s. Please provide the App ID to ensure the plugin functions correctly on the site.', RCS_TEXT_DOMAIN ),
						'<strong>',
						'</strong>'
					);
					?>
